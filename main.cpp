...
const size_t WIDTH=1404;
const size_t HEIGHT=672;
...
int main(int argc, char **argv){
  ...
  srand(time(NULL));
   ...
   csis3700::world *world1= new world(0);
    double time = 0;
   ...
   do
   {
      ...
      world1->handle_event(ev);
 
      if(redraw && al_is_event_queue_empty(event_queue)) {
         redraw = false;
         time += 1/FPS;

         world1->advance_by_time(1/FPS);
   ...
   } while(!world1->should_exit() && ev.type != ALLEGRO_EVENT_DISPLAY_CLOSE);
